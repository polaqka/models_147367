...
print('Simulating mean open time for IP3R model (Fraiman and Dawson 2004)...')
...
for ip in range(ip_concs.size):
    for i in range(ca_concs.size):
        ...
        for j in range(NITER):
            sim.reset()
            sim.setPatchSpecCount('ER_memb', 'A00', 1)
...
numpy.savetxt('results/ip3r_fd_mean_open_time.dat', res_reshaped)
numpy.savetxt('results/ip3r_fd_open_times.dat', all_open_times)
