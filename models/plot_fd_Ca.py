...
fd = np.loadtxt('results/ip3r_fd_res_ca.dat')
fd_ca_concs = np.loadtxt('results/ip3r_fd_ca_concs.dat')

fd_1 = np.loadtxt('results/ip3r_fd_res_ca_1.dat')
fd_ca_concs_1 = np.loadtxt('results/ip3r_fd_ca_concs_1.dat')

plt.figure()
plt.plot(fd_ca_concs*1e6, fd[:,0], linewidth = 2, color = 'r', label = 'original')
plt.plot(fd_ca_concs_1*1e6, fd_1[:,0], linewidth = 2, color = 'g', label = 'sustainable')
plt.xscale('log')
...
plt.savefig('charts/P0_Ca_fd.jpg', format = 'jpg')
